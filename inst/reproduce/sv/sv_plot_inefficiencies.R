...
# set this to load the results fresh
force_load <- F
k <- 20
...
# load the small file only
print('loading results...')
load(hkm_small_save_file)
...
sd_err <- sqrt(prob_arr*(1-prob_arr)/R_hkm)
...
plot_indices <- c(1,3,5)

pm_quantile_u <- t(pm_quantile_mat+2*pm_quantile_sd)[1:n_data,plot_indices]
pm_quantile_l <- t(pm_quantile_mat-2*pm_quantile_sd)[1:n_data,plot_indices]


ggplot_df1 <- data.frame(t(p_k_quantile_mat[plot_indices,1:n_data]),x1)
names(ggplot_df1)[1:length(plot_indices)] <- N_arr[plot_indices]
ggplot_df1_m <- melt(ggplot_df1,id.vars='x1')

erbar_df1 <- data.frame(t(pm_quantile_mat)[1:n_data,plot_indices[1]],pm_quantile_l[1:n_data,1],pm_quantile_u[1:n_data,1],x2)
erbar_df2 <- data.frame(t(pm_quantile_mat)[1:n_data,plot_indices[2]],pm_quantile_l[1:n_data,2],pm_quantile_u[1:n_data,2],x2)
erbar_df3 <- data.frame(t(pm_quantile_mat)[1:n_data,plot_indices[3]],pm_quantile_l[1:n_data,3],pm_quantile_u[1:n_data,3],x2)
# erbar_df4 <- data.frame(t(pm_quantile_mat)[1:n_data,plot_indices[4]],pm_quantile_l[1:n_data,4],pm_quantile_u[1:n_data,4],x2)
names(erbar_df1)[1:3] <- c('q','q_l','q_u')
names(erbar_df2)[1:3] <- c('q','q_l','q_u')
names(erbar_df3)[1:3] <- c('q','q_l','q_u')
# names(erbar_df4)[1:3] <- c('q','q_l','q_u')
ggplot_df2 <- data.frame(t(pm_quantile_mat[plot_indices,1:n_data]),x2)
...
g <- ggplot() + 
  ...
  # geom_errorbar(data=erbar_df4,aes(x=x2,ymin=q_l,ymax=q_u),width=0.2) +
  scale_x_continuous(breaks = 0:100,minor_breaks = NULL)+
  scale_y_log10(breaks =breaks ,minor_breaks=minor_breaks) + 
  scale_shape_manual(values=c(17, 16, 3,9))+
  ylab(TeX('$P\\[\\tau\\geq n\\]$')) +
  xlab('n') +
  guides(shape=guide_legend(title="N"))
g
name <- ('tailprob_sv.pdf')
ggsave(name,plot=g,width=7,height=5)



### plot variance of estimators
### we will want to plot also the variance of the log-likelihood
N_plt <- N_arr
v_plt <- est_var*N_arr
sd_u <- (1+3*sqrt(2/(n_var-1)))*est_var*N_arr
sd_l <- (1-3*sqrt(2/(n_var-1)))*est_var*N_arr

name <- paste('varlogz_sv.pdf',sep="")
ggplot_df = data.frame(N=N_plt,value=v_plt,l=sd_l,u=sd_u)

# breaks <- 10^(-10:10)
# minor_breaks <- rep(1:9, 21)*(10^rep(-10:10, each=9))
minor_breaks <- c(seq(0.1,0.5,0.1),seq(1,5,1))
g <- ggplot(data=ggplot_df) +
  geom_point(aes(x=N,y=value),cex=1) +
  geom_line(aes(x=N,y=value),linetype=2) +
  geom_errorbar(aes(x=N,ymin=l,ymax=u),width=20) +
  # scale_y_continuous(breaks =breaks ,minor_breaks=minor_breaks) +
  # scale_y_log10(breaks =minor_breaks)+# ,minor_breaks=minor_breaks) +
  ylab('estimated inefficiency') +
  theme(legend.title = element_blank())
g
...
### plot inefficiency against sigma
N_plt <- N_arr
normalised_var <- est_var*(K_max-k+1)/var_pi_h
v_plt <- normalised_var/varlogz_arr
sd_u <- (1+3*sqrt(2/(n_var-1)))*normalised_var/varlogz_arr
sd_l <- (1-3*sqrt(2/(n_var-1)))*normalised_var/varlogz_arr

name <- paste('varlogz_sv.pdf',sep="")
ggplot_df = data.frame(sigma=varlogz_arr^0.5,value=v_plt,l=sd_l,u=sd_u)

# breaks <- 10^(-10:10)
# minor_breaks <- rep(1:9, 21)*(10^rep(-10:10, each=9))
minor_breaks <- c(seq(0.1,0.5,0.1),seq(1,5,1))
g <- ggplot(data=ggplot_df) +
  geom_point(aes(x=sigma,y=value),cex=1) +
  geom_line(aes(x=sigma,y=value),linetype=2) +
  geom_errorbar(aes(x=sigma,ymin=l,ymax=u),width=min(diff(varlogz_arr^.5))/10) +
  ylab(TeX('$(m-k+1)\\cdot \\frac{V\\[H_{k:m}\\]}{V_{\\pi}\\[h\\]}   /  \\sigma^2$')) +
  xlab(TeX('$\\sigma$'))
  theme(legend.title = element_blank())
g

name <- 'sv_ineff_against_sigma.pdf'
ggsave(name,plot=g,width=7,height=5)
...
### we will want to plot also the variance of the log-likelihood
N_plt <- N_arr
v_plt_ll <- est_var_ll*(N_arr^2)
sd_u_ll <- (1+3*sqrt(2/(n_var-1)))*est_var_ll*(N_arr^2)
sd_l_ll <- (1-3*sqrt(2/(n_var-1)))*est_var_ll*(N_arr^2)

name <- paste('varlogz_sv.pdf',sep="")
ggplot_df = data.frame(N=N_plt,value=v_plt_ll,l=sd_l_ll,u=sd_u_ll)
