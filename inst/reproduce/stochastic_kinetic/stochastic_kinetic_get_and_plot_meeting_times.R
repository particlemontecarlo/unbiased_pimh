...
###
cores_requested <- min(35,detectCores()-1)
...
name <- paste('stochastic_kinetic_mt.pdf',sep="")
...
v_plt <- varlogz_arr
name <- 'varlogz_stochastic_kinetic.pdf'
...
name <- 'stochastic_kinetic_estimated_mt.pdf'
ggsave(name,plot=g,width=7,height=5)
...
x3 <- c(1:n_data)+c(rep(delta_offset,7),rep(2*delta_offset,n_data-7))

plot_indices <- c(1,2,3)
...
erbar_df3 <- data.frame(t(pm_quantile_mat)[1:n_data,plot_indices[3]],pm_quantile_l[1:n_data,3],pm_quantile_u[1:n_data,3],x3)
...
g <- ggplot() + 
  ...
  geom_point(data=erbar_df1,aes(x=x2,y=q),size=0.4) +
  geom_point(data=erbar_df2,aes(x=x2,y=q),size=0.4) +
  geom_point(data=erbar_df3,aes(x=x3,y=q),size=0.4) +
  
  geom_errorbar(data=erbar_df1,aes(x=x2,ymin=q_l,ymax=q_u),width=0.2,position=position_dodge(width=1)) +
  geom_errorbar(data=erbar_df2,aes(x=x2,ymin=q_l,ymax=q_u),width=0.2,position=position_dodge(width=1)) +
  geom_errorbar(data=erbar_df3,aes(x=x3,ymin=q_l,ymax=q_u),width=0.2,position=position_dodge(width=1)) +
...
g
name <- ('tailprobstochastic_kinetic.pdf')
ggsave(name,plot=g,width=7,height=5)
