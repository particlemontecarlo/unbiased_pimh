...
for(i in 1:M){
  print(i)
  xref <- CPF(nparticles, gss, theta, observations,ref_trajectory=xref,with_as=T)$trajectory
  xrefs[i,]=xref[1,]
}
...
name <- 'inst/exps/gss/low_T_low_K_est_var.pdf'
pdf(name)
...
name <- 'inst/exps/gss/low_T_low_K_est_var_unweighted.pdf'
pdf(name)
...
name <- 'inst/exps/gss/low_T_meeting_times.pdf'
pdf(name)
...
name <- 'inst/exps/gss/low_T_large_K_est_var.pdf'
pdf(name)
...
name <- 'inst/exps/gss/low_T_large_K_est_var_unweighted.pdf'
pdf(name)
